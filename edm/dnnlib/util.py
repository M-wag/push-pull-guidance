...
def import_net_from_url(net: str, *, verbose=True, device="cpu"):
    ...
    import sys
    # Pickled models reference torch_utils.persistence by its original top-level module path.
    # We register edm's copy under that path so pickle can find the classes without requiring
    # torch_utils to exist outside of edm.
    from .. import dnnlib as _dnnlib
    sys.modules.setdefault('dnnlib', _dnnlib)
    sys.modules.setdefault('torch_utils', _torch_utils)
    sys.modules.setdefault('torch_utils.persistence', _torch_utils.persistence)
    if isinstance(net, str):
        with open_url(net, verbose=verbose) as f:
            data = pickle.load(f)
        net = data['ema']
        # net = update_EDM(net) # Needed when changing trainings/networks code
